...
gemini_ensure_keychain() {
  ...
  if [ -n "$password" ] &&
    ...
    "$security_cmd" create-keychain -p "$password" "$staging/login.keychain-db" >/dev/null 2>&1 &&
    "$security_cmd" set-keychain-settings -u "$staging/login.keychain-db" >/dev/null 2>&1; then
