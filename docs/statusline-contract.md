...
| # | Step | Effect |
|---|---|---|
| 1 | files counter and folder debt | drop the dim `+N~M-Kf` block beside the `+A/-D` numbers (a files-only render, the sole evidence a tree is dirty, survives) and the `⟟N` folder debt — a number nobody acts on within the second is the first thing a narrow row can lose |
