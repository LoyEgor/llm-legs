...
| 7 | the verdict's autonomous dot | The dot loses the space after it (`● 7` → `●7`) and nothing else on the strip has a word left to shorten here: neither the run counter nor the verdict carries one, and their marks (`✓`, `✗`, `~`, `?<why>`) are the state rather than decoration. The gate's own numbers are never rewritten |
