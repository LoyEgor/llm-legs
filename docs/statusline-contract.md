...
| `review` | `<state_dir>/receipts/<repoName>__<repoHash>.json` written by `review-bench`, the active repository's `HEAD^{tree}`, and `git status --porcelain` | Every render resolves the active repository, reads its receipt, compares the committed tree, and reads worktree/index status; commits, edits, staging, untracked files, branch/worktree switches, and changes from any other process are visible on the next render | A matching clean tree with `receipt.errored > 0` renders dimmed; a missing, malformed, wrong-repository, or unreadable receipt, failed Git query, different `HEAD` tree, or dirty index/worktree all render normally. A dirty tree whose content happens to equal the reviewed tree is an accepted false positive | Removed only while `receipt.tree` equals `HEAD^{tree}`, `git status --porcelain` is empty, and `receipt.errored == 0` |
| review tier `⚖ review T<N>` | `review-bench suggest --repo <the active repository root>`; the command's `tier: T<N>` line is the only tier calculation, and the cache is keyed on that root | Every render reads the cache; a background `suggest` refresh starts when its cache is >5s old | Cache is shown only through 15s old, so the tier lags the working tree by up to that window — it states what the last probe measured, never that the tree is unchanged since; a slow, failing, unavailable, or unparseable command writes an empty result and leaves the segment absent | No active git repository, no `tier:` result (including clean worktrees), or stale/absent cache → nothing |
