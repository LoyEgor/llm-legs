...
| worker `w:<name>` + prediction/pin/tier | `~/.claude/worker-model`; for `auto` and unpinned Gemini the `worker-pick.line.<acct>` cache (Codex + claudeb + Gemini candidates); for Codex model label: `~/.codex/config.toml` line `model = "gpt-X-<label>"` (last dash-segment, fallback "sol"); for Codex accounts the local mirror of `~/.llm-limits.json`; for claudeb `.claudeb-state`; Gemini model/effort come from `gemini_model`/`gemini_effort` | Every render; the prediction cache is refreshed in the background when >90s stale | Prediction lines are served from a ≤90s cache (background refresh); pin/sel shown only if resolvable; Codex model label derives from `~/.codex/config.toml` every render; Gemini knobs are read live each render | `w:<name>` always shown; `?` when the worker key or predicted account is unknown |
