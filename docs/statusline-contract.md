...
| worker `w:<name>` + prediction/pin/tier | `~/.claude/worker-model`; for `auto` and unpinned Gemini the `worker-pick.line.<acct>` cache (Codex + claudeb + Gemini candidates); for Codex model label: `~/.codex/config.toml` line `model = "gpt-X-<label>"` (last dash-segment, fallback "sol"); for Codex accounts the local mirror of `~/.llm-limits.json`; for unpinned claudeb the same `worker-pick.line.<acct>` cache (`cb<mark><acct>` field) — never `.claudeb-state`, which records only the last profile launched and would predict a stale account; Gemini model/effort come from `gemini_model`/`gemini_effort` | Every render; the prediction cache is refreshed in the background when >90s stale | Prediction lines are served from a ≤90s cache (background refresh); pin/sel shown only if resolvable — an absent or unparsable cache renders `?` rather than a stale account; Codex model label derives from `~/.codex/config.toml` every render; Gemini knobs are read live each render | `w:<name>` always shown; `?` when the worker key or predicted account is unknown |
