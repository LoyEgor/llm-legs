...
| live bench `⚖ <label>` | `claude-review-bench/<sid>/running` marker written by `review-bench-hook` (PreToolUse) | Render reads marker mtime and verifies `pgrep -f 'review-bench run'` finds a live process each render | Marker mtime >2700s (45 min) → hidden (presumed stale/crashed); process not found → hidden; marker absent → nothing | Marker removed by hook (PostToolUse) or cleaned on stale timeout; label format: "bench <commitish>: <raters>" or "bench <commitish>: auto <N>" |
