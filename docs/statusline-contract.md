...
| merged review `review [T<N>]` or dim `review` | A receipt at `<state_dir>/receipts/<repoName>__<repoHash>.json` plus the active repository's `HEAD^{tree}` and `git status --porcelain`; `review-bench suggest --repo <the active repository root>` cached by repository root. The segment sits immediately after the repository/branch/diff/files cluster and before the worker segment | Every render resolves and checks the receipt, reads the suggest cache, and starts a background suggest refresh when that cache is >5s old; commits, edits, staging, untracked files, branch/worktree switches, and other-process changes are visible on the next render | A matching clean receipt suppresses suggest state; if its `errored` is nonzero the segment is dimmed instead. Otherwise the repository is unreviewed. Suggest tiers are shown only from a cache at most 15s old; stale, absent, failing, or unparseable suggest results fall back to bare `review` | Removed while the receipt repository and tree match the active clean `HEAD`, `git status --porcelain` is empty, and `receipt.errored == 0`; otherwise dim partial-receipt or unreviewed state occupies the slot |
