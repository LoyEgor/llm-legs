...
|---|---|---|---|---|
| model + effort | statusline JSON stdin (`.model.display_name`, `.effort.level`) | Every render (harness re-sends JSON, 5s) | Model and effort render; worker Fast Mode is deliberately omitted because this shared line cannot distinguish worker launches from ordinary chat/CCR. | model always shown; effort suffix only if present; Full words (`Fable 5 high`) at full width; fit step 6 abbreviates model and effort (`FB5 hi`, see Progressive fit) |
| `<account>` (magenta, no `cb:` prefix) | `CLAUDEGPT_ACCOUNT` for gateway launches; otherwise `CLAUDE_LIMITS_ACCOUNT` / `CLAUDE_CONFIG_DIR` basename | Every render | Not dimmed | Absent when `acct=main` (plain non-claudeb session); gateway `main` stays visible; truncated from the right down to a four-character floor as the last fit step, never dropped |
...
| Writer | Kind | Path |
|---|---|---|
| `statusline-workdir-hook` SessionStart, only while the journal is missing or empty | `seed` | the session cwd |
...
Nothing else writes: a `Read`, a read-only command (`(cd /x && git status)`, `git -C /x log`), a
write with no cd and no `-C`, `git worktree remove`, and a subagent's Bash or dispatch move nothing.
...
Shared abbreviation rule (head model only from step 6): model = first letter
