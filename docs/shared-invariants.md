...
| c | worker-pick cache line format | `cx<mark><acct>·<eff> cb<~|@><acct>·<model>·<eff>` (producer printf `cx%s%s·%s %s·%s·%s`, `cb` prefixes `cb~`/`cb@`/`cb~?`) | producer `bin/worker-pick` `write_cache`; consumer `bin/statusline.sh` reads `worker-pick.line.<acct>` verbatim |
