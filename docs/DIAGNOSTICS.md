...
| Wrong account picked | `cat ~/.claude-profiles/.claudeb/.claudeb-state` (last launched, drives `*`/`●`/`cb:`), `~/.claude-profiles/.claudeb/disabled` (out of the worker pool), `cat ~/.cache/worker-pick.line.<acct>` (what the statusline's `w:cb` predicts), then inspect `worker-pick` output |
