...
- `bin/review-bench` — blind review benchmark runner. Antigravity cells are explicit-only (`--raters agy-pro-<low|high>`, `agy-flash36-<low|medium|high>`, or `agy-flash35-<low|medium|high>`, with optional `-skill`) and use the single `agy` account without worker-pick or llm-limits affordability gates. Every run executes in a sealed clone and keeps `raw-<rater>.md`, `agy-<rater>.log`, `usage-<rater>.jsonl`, and normalized `findings-<rater>.jsonl` under the run directory. Empty, malformed, error, and headless permission-denied output fail closed. Skill cells invoke the imported official Gemini CLI `code-review` extension; its Markdown is adapted to the shared finding schema.
...
| macOS "A keychain cannot be found to store antigravity" dialog during a refresh | A Gemini profile HOME lost its keychain link: `HOME=~/.gemini-profiles/<name> security default-keychain` must print the real `login.keychain-db`. `ls -l ~/.gemini-profiles/<name>/Library/Keychains` should be a symlink to `~/Library/Keychains`; `bin/geminib help` re-creates a missing or dangling one, but never replaces a real directory sitting there — move that aside yourself first. Only agy's token-refresh path raises the dialog, so a probe on a still-valid token looks clean |
...
2. **Header 429** — has `anthropic-ratelimit-unified-reset` or `retry-after`. `markRejected()` walls the **request's own scope** (general or fable — never both) until the header-specified time, reason=`header`. This is real quota, not capacity noise. A rejection is the ONLY thing headers say about the weekly bucket: they publish no weekly percentage, so a quota block is a wall with the vendor's reset time and never a written percentage (shared-invariants n). `anthropic-ratelimit-unified-status` values other than `allowed` (e.g. the warning class) plus `representative-claim: seven_day` mean "the weekly bucket is the one closest to its limit" — informational, not a block. Reading it as one caused the 2026-07-24 incident: a flat `100` was stamped into the weekly bucket of `com`/`notcom` (real readings were 76% / 84%), which walled the general scope for hours (`hold ... outcome=503`) while the actual exhausted bucket was Fable-weekly. Symptom to recognize: `walls: []` in `/claudebd/status` while `all_walled_until` is set and the weekly cell reads exactly `100%` with `origin: "headers"`.
...
- `bash tests/test_review_bench.sh` — hermetic review-bench grammar, normalization, sealed-clone, Antigravity bare JSONL/error-envelope, per-run usage, and `/code-review` Markdown adapter tests; all agy calls use fixtures.
...
- **Red** = rotation-blocked, independent of the checkbox. The daemon is the single source through each account's `rotation.blocked`: `auth`, `wall`, `limit-5h`, or `limit-weekly` blocks the account title plus 5h/weekly rows; the scope-aware `fb` row uses `blocked.fable`, which may additionally be `limit-fable`, `plan` (static gate: the account's `account-tiers` tier is below `CLAUDEBD_FABLE_MIN_TIER`=100, i.e. the $20/Pro plan has no Fable), or `no-capability` (affirmative live evidence: a fable-scope 403 plan rejection, persisted in `daemon-state.json` as `fableIncapable:{at,status}` and decaying back to capable after `CLAUDEBD_FABLE_EVIDENCE_TTL_S`=24h). The daemon NEVER derives fable capability by reading `rotation.usable.fable` back from `~/.llm-limits.json` — that store field is written from the daemon's own status, so reading it back froze one transient wall into a permanent `no-capability`. A fable-only blockage does not color the account title or general rows. Dim-red when both blocked and stale. If the daemon is unreachable, the collector omits `rotation` and the renderer does not infer blockage.
