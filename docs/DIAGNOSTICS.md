...
| macOS "security wants to use the login keychain" dialog during a Gemini run or refresh | OPEN, no fix yet — a profile keychain is locked and agy's access to it prompts. Measured 2026-07-27: `unlock-keychain` on a file named `login.keychain-db` fails by absolute path (macOS resolves the basename to the session keychain), and unlocking a renamed copy is not carried over when the file is renamed back — the keychain is locked again at its own path, so a rename-unlock-rename helper is a no-op. `create-keychain` leaves a new keychain unlocked until the next boot, which is why a fresh profile looks fixed for a while; verify any candidate fix only across a reboot. The abandoned attempt is on branch `keychain-attempt-2026-07-27`. Keep `security list-keychains -d user` free of profile keychains: a locked entry there makes unrelated session lookups prompt too |
