...
| macOS "security wants to use the login keychain" dialog during a Gemini run or refresh | `security list-keychains -d user` must not contain Gemini profile or `.staging.*` keychains, and `HOME=~/.gemini-profiles/<name> security list-keychains -d user` must contain only that profile's `login.keychain-db`. Any `geminib` run heals the real search list and unlocks the profile keychain before agy starts. Do not replace that with a direct `security unlock-keychain .../login.keychain-db`: macOS misroutes even the absolute path because of the basename, so the helper temporarily renames it under a lock for the unlock. |
