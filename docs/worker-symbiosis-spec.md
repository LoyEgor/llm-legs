...
- `reset_credits` (integer, ↻N in the table's CR column) from any vendor that reports one —
  Codex, grok and Claude today — plus `plan_type` and `reset_credits_expires_at`.
...
- R5 extra capacity beyond remaining_now, two distinct sources:
  ...
  b. `reset_credits` (count, Codex, grok and Claude): each credit ≈ one full 100% quota refill on top of
