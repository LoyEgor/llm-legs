...
`bin/worker-pick` (the `*` own-row marker and the `worker-pick.line.codex@<account>` prediction
file), `bin/claude-resume-timer` (times the resume off the codex five-hour window),
