...
jq -e '.vendors.claude.five_hour.used_pct == 19 and .vendors.claude.weekly.used_pct == 53' <<<"$out" >/dev/null || fail "Claude values mismatch"
...
echo "PASS: schema, small-file fallback, truncated boundary, walls, plain output, atomic cache, missing exit 3"
exit 0
